...
impl Blockchain {
    ...
    fn is_block_valid(&self, new_block: &Block, latest_block: &Block) -> bool {
        ...
        {
            println!("Block with id {} is invalid hash", new_block.id);
            return false;
        }
        true
    }
    ...
    fn chain_selector(&self, local: Vec<Block>, remote: Vec<Block>) -> Option<Vec<Block>> {
        ...
        match (is_local_valid, is_remote_valid) {
            ...
            (false, false) => {
                println!("Both the local and the remote copy are not valid");
                None
            },
        }
    }
    ...
    fn mine_block(id: u64, timestamp: i64, previous_hash: &str, data: &str) -> (u64, String) {
        ...
        loop {
            let block_string: String =
                format!("{}{}{}{}{}", id, previous_hash, data, timestamp, nonce);
...
fn main() {
    ...
    let new_block: Block = Block::new(3, new_BC.blocks[1].hash.to_owned(), "Vortex".to_string());
    new_BC.try_add_block(new_block);

    let new_block: Block = Block::new(4, new_BC.blocks[2].hash.to_owned(), "Himashree".to_string());
    new_BC.try_add_block(new_block);
